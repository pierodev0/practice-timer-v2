...
import { secToMin } from '../src/infrastructure/services/export.js';
...
describe('downloadDayXLSX', () => {
  ...
  it('shows alert if ExcelJS is not loaded', async () => {
    ...
    const { downloadDayXLSX } = await import('../src/infrastructure/services/export.js');
...
describe('downloadMonthXLSX', () => {
  ...
  it('generates a workbook with multiple day sheets', async () => {
    ...
    const { downloadMonthXLSX } = await import('../src/infrastructure/services/export.js');
    const groups = {
      '2026-07-04': [
        {
